...
int main() {
    ...
    t_assert(state->supplyCount[copper] == 60 - (7 * numPlayers), "incorrect number of copper");
    ...
    t_assert(state->handCount[state->whoseTurn()] == 5, "incorrect number of cards in hand");
    t_assert(state->supplyCount[curse] == 10, "incorrect number of curses");

    //bad test, with an invald kingdom card
    kingdomCards[0] = -1;
    state = newGame();
    int ret = initializeGame(state);
